...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    await Store.Tickets.fetchAll?.();
  } catch (e) {
    console.warn("fetchAll not required or not implemented:", e);
  }

  allTickets = Store.Tickets.getAll();
  renderStats();
  renderUsers();
});
              ...
              <td>
                <div style="display:flex;align-items:center;gap:10px;">
                  ...
                  <div>
                    ...
                    ${
                      isSelf
                        ? `<span style="font-size:10px;background:var(--orange);color:#fff;padding:2px 6px;border-radius:4px;margin-left:6px;">You</span>`
                        : ''
                    }
                  </div>
                </div>
...
async function toggleRole(id, currentRole) {
  ...
  await Store.Users.updateRole(id, newRole);
